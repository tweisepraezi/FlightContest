...
* link:rules/DE_Rallyeflug_2026.pdf[Wettbewerbsordnung Rallyeflug Deutschland^]
* link:rules/DE_ANR_2026.pdf[Wettbewerbsordnung Air Navigation Race Deutschland^]
* link:rules/DE_Landewertung_2026.pdf[Regelwerk Landewertung Deutschland^]
...
"Koordinaten importieren" aus einer Text-Datei hinzugefügt werden (einschl. Eigenschaften):
 ...
 SC1,  Lat 49° 01.83100' N, Lon 009° 55.43200' E, Alt 2063ft, Gate 1.0NM
...
. Strecke mit TO-Punkt anlegen (Strecken -> Neue Strecke)
+
...
. Karte des Gebietes um den TO-Punkt für Task-Creator erzeugen (Abstand zum Rand bei Rallye- und Präzisionsflug: 420mm, bei ANR: 297mm)
+
...
. Import der Strecke (Strecken -> Import Task-Creator-Strecke)
+
...
("GPX-Export (mit Halbkreis-Tore)" und folgendem "Import FC-Strecke")
...
(TP1 -> SC, z.B. "TP1, Gate 1.0NM" -> "SC, Gate 1.0NM" (SC muss nicht nummeriert werden))
...
. Gebiet um TO-Punkt auswählen

. Strecke mit TO-Punkt anlegen (Strecken -> *Neue Strecke*)

. Wettbewerbs-Karte mit TO-Punkt drucken (Strecke -> *OSM-Wettbewerbs-Karte*) +
Dabei TO-Punkt an eine gewünschte Position legen (rechts oben, links unten o.ä.).
...
Check-Punkt-Einstellungen eines krummen Streckenabschnittes fehlerhaft::
...
Check-Punkt-Einstellungen eines landschaftlichen Streckenabschnittes fehlerhaft::
Bei landschaflichen Abschnitten müssen die Flags "Keine Zeitauswertung", "Keine Torauswertung" und "Check-Punkt ignorieren" der UZKs aktiviert sein.
...
Im Luftraum-Textfeld einer Strecke können beliebig viele Lufträume eingetragen sein. 
  ...
  #coord,text:AREA 2500ft-FL100,fillcolor:gray,textcolor:black,coords:...
  coord,text:CTR BUECHEL GND-4100ft,coords:...
  #coord,text:DÜSSELDORF 1000ft-2500ft,fillcolor:gray,textcolor:black,coords:...
  coord,text:ED-R5 BIBLIS H24 GND-2300ft,fillcolor:red,textcolor:red,coords:...
...
* "_Online-Karte um Flughafen erzeugen_" erzeugt eine Karte mit TO-Punkt in der Mitte für OSM-Online-Anzeige (Abstand zum Rand bei Rallye- und Präzisionsflug: 420mm, bei ANR: 297mm).
* "_Task-Creator-Karte um Flughafen erzeugen_" erzeugt eine Karte mit TO-Punkt in der Mitte für Verwendung im Task-Creator (Abstand zum Rand bei Rallye- und Präzisionsflug: 420mm, bei ANR: 297mm).
...
Es werden Parcours mit bis zu 4 Strecken unterstützt.
...
. Karte des Gebietes um den TO-Punkt für Task-Creator erzeugen (Abstand zum Rand: 297mm)
+
OSM-Wettbewerbs-Karte -> Task-Creator-Karte um Flughafen erzeugen
+
Zuvor "Flugplätze um Flugplatz suchen" durchführen. +
...
. Strecken im Task-Creator konstruieren
+
Task-Creator starten (Karten -> "Flugplatz-Name (Task Creator)" -> Task-Creator -> …) +
+
Im gestarteten Task-Creator ist eine "Karten-Url" auf die erzeugte Karte voreingestellt und kann mit "Laden" geladen werden. +
...
. Mehrfach-Strecken-Parcour konfigurieren (optional)
+
Strecken-Einstellungen der 1. Strecke öffnen +
Bis zu 3 weitere Strecken der 1. Strecke zuordnen und einen Parcour-Namen festlegen.

. A4-Wettbewerbs-Karte erzeugen
+
...
Hier können dann auch weitere Einstellungen vorgenommen werden, wie die weiteren Strecken bei Mehrfach-Strecken-Parcours, das Flugplan-Design, die Anzeige von Werten im Flugplan und geänderte Werte für Startbahn-Richtung und An- und Abflug-Flugzeiten. +
...
|timetablelist|value [#crew.num]|num crew aircraft tas team resultclass shortresultclass route planningtime planningtimebefore takeofftime version
|legtimelist|value [#tas]|tas legtime
|timetablejudgelist|value [#crew.num]|num crew aircraft aircrafttype aircraftcolor tas team resultclass shortresultclass route planningtime planningtimebefore planningendtime takeofftime sptime tptime fptime landingtime arrivaltime submissiontime empty1 empty2 empty3 empty4
...
<<fc.adoc#configuration-fcmaplocal>> +
<<fc.adoc#configuration-fcmapstyles>> +
<<fc.adoc#configuration-openaip>> +
...
Errichtung eines eigenen Druck-Servers: https://github.com/tweisepraezi/FlightContest/blob/master/fcmapsserver/_install.txt


[[configuration-fcmaplocal]]
=== Lokalen Karten-Druck-Server konfigurieren
 flightcontest {
   maps {
     // contoursources = "srtm3" // Quelle von Contour-Daten
                                 // Default: view3
     srtm {
        // username = "TODO"     // User-Name für Zugriff auf SRTM-Contour-Daten
        // password = "TODO"     // Passwort für Zugriff auf SRTM-Contour-Daten
     }
     postgresql {
        // psqlexe = "TODO"   // Pfad der PostgreSQL-Konsole
                              // Default: C:/Program Files/PostgreSQL/17/bin/psql.exe
        // host = "TODO"      // Adresse des lokalen PostgreSQL-Servers (DNS-Name oder IP-Adresse)
                              // Default: automatisch ermittelt
        // port = 5433        // Port, über den der lokale PostgreSQL-Server angesprochen wird
                              // Default: 5432 oder 5433, automatisch ermittelt
        // username = "TODO"  // User-Name für Zugriff auf lokalen PostgreSQL-Server
                              // Default: postgres
        password = "TODO"     // Passwort für Zugriff auf lokalen PostgreSQL-Server
     }
   }
 }

[[configuration-fcmapstyles]]
=== Anzeige-Stil von Lufträumen und anderer Strecken im ANR-Parcour
 flightcontest {
   maps {
     styles { 
       // Luftraum-Stil 
       // airspaceICAO8type1 = "TODO" // Default ED-R: fillcolor:red,textcolor:red
       // airspaceICAO8type6 = "TODO" // Default RMZ: fillcolor:steelblue,textcolor:black
       // airspaceDefault = "TODO" // Default: fillcolor:gray,textcolor:black
       //
       // Stil eines zusätzlichen Luftraum-Importes
       // airspaceImport = "TODO" // Default: fillcolor:blue,textcolor:black
       //
       // Stil des Rechtecks der anderen Strecken im ANR-Parcour
       // anrRouteArea = "TODO" // Default: fillcolor:gray,textcolor:black
     }
   }
 }
