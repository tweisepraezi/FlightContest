...
Activate new databases *gis_eu* and *lowzoom_eu*
 ...
 psql -d postgres -c 'ALTER DATABASE lowzoom2 RENAME TO lowzoom_eu;'
...
Load contour data into contour database (all generated pbf files must be loaded together in one step)
 ...
 osm2pgsql -d contours_new --slim --cache 12000 --number-processes 10 --style ~/fcmaps/osm2pgsql/contours.style ~/data/srtm/lon*.pbf
