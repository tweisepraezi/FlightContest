...
Download data of countries (if needed)
...
Load initial OSM data into OSM database (~/data/osm/*.pbf -> gis2)

 osm2pgsql -d gis2 --slim --cache 12000 --number-processes 10 --style ~/fcmaps/osm2pgsql/fcmaps.style ~/data/osm/europe-latest.osm.pbf

Wait... (22h)
 
Load more OSM data into OSM database (~/data/osm2/*.pbf -> gis2)

 for pbffile in ~/data/osm2/*.pbf; do osm2pgsql -d gis2 --append --slim --cache 12000 --number-processes 10 --style ~/fcmaps/osm2pgsql/fcmaps.style $pbffile; done
