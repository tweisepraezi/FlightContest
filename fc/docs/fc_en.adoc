...
. Create route with TO point (Routes -> New Route)
+
...
. Create a map of the area around the TO point for Task Creator (distance to edge for Rally and Precision flying: 420 mm, for ANR: 297 mm)
+
...
. Import route (Routes -> Import Task Creator route)
+
...
(run "GPX export (with semi-circle gates)", and run then "Import FC route").
...
(TP1 -> SC, e.g. "TP1, Gate 1.0NM" -> "SC, Gate 1.0NM" (SC does not need to be numbered))
...
. Select area around TO point

. Create route with TO point (Routes -> *New route*)

. Print competition map with TO point (route -> *OSM Contest Map*) +
Place TO point at a desired position (top right, bottom left or similar).
...
* "_Generate online map around airport_" creates a map with TO point in the center for OSM online map display (distance to edge for Rally and Precision flying: 420 mm, for ANR: 297 mm).
* "_Generate Task Creator map around airport_" creates a map with TO point in the center for use in the Task Creator (distance to edge for Rally and Precision flying: 420 mm, for ANR: 297 mm).
...
Parcours with up to 4 routes are supported.
...
<<fc_en.adoc#anr-routes-construction>> +
...
. Create a map of the area around the TO point for Task Creator (distance to edge: 297 mm)
+
OSM Contest Map -> Generate Task Creator map around airport +
+
Run "Search airfields around the airport" first. +
...
. Construct routes in the Task Creator
+
Start Task Creator (Maps -> "Airfield name (Task Creator)" -> Task Creator -> …) +
+
In the Task Creator that has been started, a "Map Url" is preset to the generated map and can be loaded with “Load”. +
...
. Configure multi-route parcour (optional)
+
Open route settings of 1st route +
Assign up to 3 additional routes to the first route and specify a parcour name.

. Create an A4 competition map
+
...
Further settings can also be made here, such as additional routes for multi-route parcours, the flight plan design, the display of values in the flight plan and changed values for runway direction and arrival and departure flight times. +
...
<<fc_en.adoc#configuration-fcmaplocal>> +
<<fc_en.adoc#configuration-fcmapstyles>> +
<<fc_en.adoc#configuration-openaip>> +
...
[[configuration-fcmap]]
=== Configure your own map print server
 flightcontest {
   maps {
     fcmap {
...
Setting up your own print server: https://github.com/tweisepraezi/FlightContest/blob/master/fcmapsserver/_install.txt

[[configuration-fcmaplocal]]
=== Configure local map print server
 flightcontest {
   maps {
     // contoursources = "srtm3" // Sources of contour data
                                 // Default: view3
     srtm {
        // username = "TODO"  // User name for accessing to SRTM contour data
        // password = "TODO"  // Password for accessing to SRTM contour data
     }
     postgresql {
        // psqlexe = "TODO"   // Path of PostgreSQL console
                              // Default: C:/Program Files/PostgreSQL/17/bin/psql.exe
        // host = "TODO"      // Address of the local PostgreSQL server (DNS name or IP address)
                              // Default: automatically determined
        // port = 5433        // Port through which the local PostgreSQL server is accessed
                              // Default: 5432 oder 5433, automatically determined
        // username = "TODO"  // User name for accessing the local PostgreSQL server
                              // Default: postgres
        password = "TODO"     // Password for accessing the local PostgreSQL server
     }
   }
 }

[[configuration-fcmapstyles]]
=== Display style for airspaces and other routes in the ANR parcour
 flightcontest {
   maps {
     styles { 
       // Airspace style 
       ...
       //
       // Style of an additional airspace import
       // airspaceImport = "TODO" // Default: fillcolor:blue,textcolor:black
       //
       // The style of the rectangle for all other routes of the ANR parcour
