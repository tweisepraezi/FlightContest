<html>
    <head>
        ...
        <title>${resultclassInstance.GetListTitle('fc.contest.listresults')}</title>
    </head>
    <body>
        ...
        <div class="box">
            ...
            <div class="box boxborder" >
                <h2>${resultclassInstance.GetListTitle('fc.contest.listresults')}<g:if test="${resultclassInstance.AreClassResultsProvisional(resultclassInstance.GetClassResultSettings())}"> [${message(code:'fc.provisional')}]</g:if></h2>
                <div class="block" id="forms" >
                    <g:form params="${['positionsReturnAction':positionsReturnAction,'positionsReturnController':positionsReturnController,'positionsReturnID':positionsReturnID]}" >
                        ...
                        <table>
                            ...
                            <tbody>
                                <g:each var="crew_instance" in="${Crew.findAllByContest(resultclassInstance.contest,[sort:'contestPosition'])}" status="i">
                                	<g:if test="${crew_instance.resultclass == resultclassInstance}">
	                                    <tr class="${(i % 2) == 0 ? 'odd' : ''}">
	                                        ...
	                                        <g:each var="task_instance" in="${Task.findAllByContest(resultclassInstance.contest)}">
	                                        	...
	                                        	<td>${test_instance.GetResultPenalties(resultclassInstance.GetClassResultSettings())} ${message(code:'fc.points')}<g:if test="${test_instance.AreClassResultsProvisional(resultclassInstance.GetClassResultSettings(),resultclassInstance)}"> [${message(code:'fc.provisional')}]<g:set var="test_provisional" value="${true}"/></g:if> <a href="${createLink(controller:'test',action:'crewresults')}/${test_instance.id}">${message(code:'fc.test.results.here')}</a></td>
			                                </g:each>
