class CoordResultController 
{
	...
	def gotonext = {
		def coordresult = fcService.getCoordResult(params)
		if (coordresult.instance) {
            long nextcoordresult_id = coordresult.instance.GetNextCoordResultID()
			if (nextcoordresult_id) {
                redirect(action:'edit',id:nextcoordresult_id)
            } else {
                redirect(controller:"test",action:'flightresults',id:params.testid)
            }
		}
	...
	def gotoprev = {
		def coordresult = fcService.getCoordResult(params)
		if (coordresult.instance) {
            long prevcoordresult_id = coordresult.instance.GetPrevCoordResultID()
			if (prevcoordresult_id) {
                redirect(action:'edit',id:prevcoordresult_id)
            } else {
                redirect(controller:"test",action:'flightresults',id:params.testid)
            }
        }
    ...
    def resetprocedureturn = {
        def coordresult = fcService.resetCoordResultProcedureTurn(params) 
        if (coordresult.saved) {
            flash.message = coordresult.message
            redirect(controller:"test",action:'flightresults',id:params.testid)
        } else if (coordresult.instance) {
