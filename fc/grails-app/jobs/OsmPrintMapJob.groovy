...
class OsmPrintMapJob
{
    ...
    def execute(context) {
        ...
        String print_corridorwidth = context.mergedJobDataMap.get(Defs.OSMPRINTMAP_PRINTCORRIDORWIDTH)
        boolean print_colorchanges = context.mergedJobDataMap.get(Defs.OSMPRINTMAP_PRINTCOLORCHANGES)
        try {
            osmPrintMapService.BackgroundJob(action_name, job_filename, job_id, jobid_filename, fileid_filename, png_filename, print_landscape, print_size, print_projection, print_corridorwidth, print_colorchanges)
        } catch (Exception e) {
